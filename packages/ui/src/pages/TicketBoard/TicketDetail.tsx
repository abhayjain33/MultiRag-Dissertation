...
export function TicketDetail() {
  ...
  // Initialise store when data loads
  useQuery({
    queryKey: ['ticket-init', ticketId, data],
    queryFn: () => {
  ...
  const handleWSEvent = useCallback(
    (event: import('@/types/ticket').TicketWSEvent) => {
      applyWSEvent(event);
    },
    [applyWSEvent],
  );
  ...
  if (error || !data) {
    return (
      <div className="min-h-screen bg-gray-50 flex flex-col items-center justify-center gap-3">
        ...
        <Link to="/tickets" className="text-indigo-600 hover:underline text-sm">
          ← Back to ticket list
        </Link>
      </div>
    );
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      <div className="bg-white border-b border-gray-200 px-6 py-3 flex items-center gap-4 sticky top-0 z-30">
        <Link to="/tickets" className="text-gray-400 hover:text-gray-600 flex items-center gap-1 text-sm">
